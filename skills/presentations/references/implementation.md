...
Call 预置的 `RUNTIME_*` 环境变量. Use its Node.js executable, Node.js packages, override binaries and Python executable as `RUNTIME_NODE`, `RUNTIME_NODE_MODULES`, `RUNTIME_BIN_DIR` and `RUNTIME_PYTHON`. Do not install substitutes or modify the bundled dependencies.
