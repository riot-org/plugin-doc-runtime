---
name: documents
description: "创建、编辑、审阅和批注 .docx / Word 文档。带强制的渲染校验流程:用 render_docx.py 生成页面 PNG,逐页看过确认排版无缺陷后再交付。"
---
...
Use this skill when you need to create or modify `.docx`, Word, or artifacts 并做可视化校验。

本 skill 只产出本地 `.docx`。Riot 版本里的 Google Docs 导入链路依赖它的 Drive connector,Riot 没有,已移除。
...
to depart from the template. The render gate still applies.
...
- 用 `Read` 工具打开 PNG (100% zoom) and confirm every page is clean
...
"$RUNTIME_BIN_DIR/python3" render_docx.py input.docx --output_dir out

# 2) Remove reviewer comments (finalization)
"$RUNTIME_BIN_DIR/python3" scripts/comments_strip.py input.docx --out no_comments.docx

# 3) Accept tracked changes (finalization)
"$RUNTIME_BIN_DIR/python3" scripts/accept_tracked_changes.py input.docx --mode accept --out accepted.docx

# 4) Accessibility audit (+ optional safe fixes)
"$RUNTIME_BIN_DIR/python3" scripts/a11y_audit.py input.docx
"$RUNTIME_BIN_DIR/python3" scripts/a11y_audit.py input.docx --out_json a11y_report.json
"$RUNTIME_BIN_DIR/python3" scripts/a11y_audit.py input.docx --fix_image_alt from_filename --out a11y_fixed.docx

# 5) Redact sensitive text (layout-preserving by default)
"$RUNTIME_BIN_DIR/python3" scripts/redact_docx.py input.docx redacted.docx --emails --phones
```
...
```bash
"$RUNTIME_BIN_DIR/python3" render_docx.py /mnt/data/input.docx --output_dir /mnt/data/out
# If debugging LibreOffice:
"$RUNTIME_BIN_DIR/python3" render_docx.py /mnt/data/input.docx --output_dir /mnt/data/out --verbose
# Optional: also write <input_stem>.pdf to --output_dir (for debugging/archival):
"$RUNTIME_BIN_DIR/python3" render_docx.py /mnt/data/input.docx --output_dir /mnt/data/out --emit_pdf
```
...
If you need to use LibreOffice, use the absolute path to bundled LibreOffice resolved through 预置的 `RUNTIME_*` 环境变量, never use the user's installed desktop LibreOffice even if the bundled version fails. Include this instruction and the bundled path in every subagent handoff.
