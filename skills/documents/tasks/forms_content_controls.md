...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/content_controls.py /mnt/data/template.docx wrap_placeholders \
  --output /mnt/data/template_sdt.docx
```

3. **Populate SDTs by tag**
```bash
"$RUNTIME_BIN_DIR/python3" scripts/content_controls.py /mnt/data/template_sdt.docx fill \
...
4. **Render for QA**
```bash
"$RUNTIME_BIN_DIR/python3" render_docx.py /mnt/data/filled.docx --output_dir /mnt/data/out_forms
```
...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/content_controls.py /mnt/data/template_sdt.docx list --json
```
