...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/comments_add.py input.docx --out reviewed.docx --author "Reviewer"   --add "Payment Terms=Please confirm Net 45 is acceptable."   --add "Governing Law=Prefer Delaware; any constraints?"   --ignore_case
```
Notes:
...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/comments_extract.py reviewed.docx --out comments.json
...
"$RUNTIME_BIN_DIR/python3" scripts/comments_apply_patch.py reviewed.docx patch.json --out reviewed_v2.docx
```
...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/comments_extract.py input.docx --out comments.json
```
...
"$RUNTIME_BIN_DIR/python3" scripts/comments_strip.py accepted.docx --out final_clean.docx
"$RUNTIME_BIN_DIR/python3" render_docx.py final_clean.docx --output_dir out_final_clean
```
