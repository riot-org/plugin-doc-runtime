...
1. **Inspect** how many revisions exist:
   ```bash
   "$RUNTIME_BIN_DIR/python3" scripts/accept_tracked_changes.py input.docx --mode report
   ```
2. **Accept** all tracked changes into a clean copy:
   ...
   (Or reject):
   ```bash
   "$RUNTIME_BIN_DIR/python3" scripts/accept_tracked_changes.py input.docx --mode reject --out rejected.docx
   ```
3. **Render → PNG review** (required):
   ```bash
   "$RUNTIME_BIN_DIR/python3" render_docx.py accepted.docx --output_dir out_accept
   ```
