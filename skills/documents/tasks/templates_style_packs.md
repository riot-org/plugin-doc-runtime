...
1. Apply the template parts to your doc:
   ```bash
   "$RUNTIME_BIN_DIR/python3" scripts/apply_template_styles.py --template template.dotx --target report.docx --out report_styled.docx
   ```

2. Render and review:
   ```bash
   "$RUNTIME_BIN_DIR/python3" render_docx.py report_styled.docx --output_dir out_styled
   ```
