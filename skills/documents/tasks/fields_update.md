...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/fields_report.py /mnt/data/input.docx
```
...
"$RUNTIME_BIN_DIR/python3" scripts/flatten_ref_fields.py input.docx --out ref_flattened.docx

# Materialize SEQ/REF results (e.g., caption numbers / cross-refs)
"$RUNTIME_BIN_DIR/python3" scripts/fields_materialize.py ref_flattened.docx --out fields_materialized.docx
```
