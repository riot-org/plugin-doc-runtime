...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/captions_and_crossrefs.py \
  /mnt/data/in.docx \
...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/insert_ref_fields.py \
  /mnt/data/with_captions.docx \
...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/fields_materialize.py \
  /mnt/data/with_refs.docx \
...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/fields_materialize.py /mnt/data/with_refs.docx --out /mnt/data/out.docx --only REF
```

### 4) Render and visually QA
```bash
"$RUNTIME_BIN_DIR/python3" render_docx.py /mnt/data/with_refs_materialized.docx --output_dir /mnt/data/out_caps
```
Inspect the PNGs.
