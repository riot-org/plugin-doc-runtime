...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/captions_and_crossrefs.py /mnt/data/in.docx /mnt/data/with_caps.docx --figures --tables --bookmarks
```

3) Add navigation

```bash
"$RUNTIME_BIN_DIR/python3" scripts/internal_nav.py /mnt/data/with_caps.docx --out /mnt/data/with_nav.docx
```

4) Render and verify

```bash
"$RUNTIME_BIN_DIR/python3" render_docx.py /mnt/data/with_nav.docx --output_dir /mnt/data/out_nav
```
