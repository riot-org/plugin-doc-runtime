...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/merge_docx_append.py base.docx append.docx --out merged.docx
```
...
```bash
"$RUNTIME_BIN_DIR/python3" scripts/merge_docx_append.py base.docx append.docx --out merged.docx --allow_drawings
```

## Verify
Always render and inspect:
```bash
"$RUNTIME_BIN_DIR/python3" render_docx.py merged.docx --output_dir out_merged
```
